...
import com.minko.mall.portal.component.CancelOrderSender;
import com.minko.mall.portal.dao.PortalOrderDao;
import com.minko.mall.portal.dao.PortalOrderItemDao;
import com.minko.mall.portal.dao.SmsCouponHistoryDao;
import com.minko.mall.portal.domain.*;
import com.minko.mall.portal.service.*;
...
public class OmsPortalOrderServiceImpl implements OmsPortalOrderService {
    ...
    public ConfirmOrderResult generateConfirmOrder(List<Long> cartIds) {
        ...
        //获取用户收货地址列表
        List<UmsMemberReceiveAddress> memberReceiveAddressList = memberReceiveAddressService.getList();
    ...
    public void cancelOrder(Long orderId) {
        ...
        orderLambdaQueryWrapper.eq(OmsOrder::getId, orderId)
                .eq(OmsOrder::getStatus, 0)
                .eq(OmsOrder::getDeleteStatus, 0);
        List<OmsOrder> cancelOrderList = orderMapper.selectList(orderLambdaQueryWrapper);
        ...
        if (cancelOrder != null) {
            ...
            List<OmsOrderItem> orderItemList = orderItemMapper.selectList(orderItemLambdaQueryWrapper);
            //解除订单商品库存锁定
    ...
    @Override
    public CPage<OmsOrderDetail> list(Integer status, Integer pageNum, Integer pageSize) {
        if (status == -1) {
            status = null;
        }
        UmsMember member = memberService.getCurrentMember();
        ...
        LambdaQueryWrapper<OmsOrder> orderLambdaQueryWrapper = new LambdaQueryWrapper<>();
        orderLambdaQueryWrapper.eq(OmsOrder::getDeleteStatus, 0)
                .eq(OmsOrder::getMemberId, member.getId());
        if (status != null) {
            orderLambdaQueryWrapper.eq(OmsOrder::getStatus, status);
        }
        orderLambdaQueryWrapper.orderByDesc(OmsOrder::getCreateTime);
        Page<OmsOrder> orderList = orderMapper.selectPage(page, orderLambdaQueryWrapper);
        CPage<OmsOrder> orderPage = CPage.restPage(orderList);
        //设置分页信息
        CPage<OmsOrderDetail> resultPage = new CPage<>();
